...
- Nginx 使用项目独立 PID 文件时，systemd `PIDFile` 必须与 Nginx 配置完全一致，`ExecReload`/`ExecStop` 必须通过该 unit 的 `$MAINPID` 发送信号；部署阶段函数经 `record_timed_phase` 调用时不得依赖 `set -e` 隐式传播，edge/公共 Nginx 的 start、reload 和 health 状态必须逐项显式检查并 fail-closed。
- 改完代码必须跑测试，不能只编译通过。
