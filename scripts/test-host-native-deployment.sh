...
rg -Fq 'install -d -o ubuntu -g ubuntu -m 0755 "$release_dir" || return 1' "$ARTIFACT"
rg -Fq 'install -o ubuntu -g ubuntu -m 0644 "$jar" "$release_dir/app.jar" || return 1' "$ARTIFACT"
rg -Fq 'install -o ubuntu -g ubuntu -m 0600 "$manifest" "$release_dir/artifact.manifest" || return 1' "$ARTIFACT"
