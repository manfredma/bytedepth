...
4. **staging 预发验收**（界面/视觉/布局改动必须，后端改动建议）：
   ...
   - 项目所有者通过 `https://staging.bytedepth.cn/?preview=true` 验收。**未收到明确「staging 验收通过」不得合并 `main`。** 不带预览标记的公网请求会 301 到生产，该参数不是安全认证。
...
# 项目所有者在 https://staging.bytedepth.cn/?preview=true 验收通过后，再创建并合并 PR
```
