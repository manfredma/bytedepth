...
- **staging integration 必须给共享 JVM 服务留内存**：Surefire fork 峰值曾达到约 664 MiB；4 GiB 多项目主机的 swap 已满时，低内存预检会让 global OOM killer 杀掉测试 fork。runner 停 app 前至少要求 768 MiB、停后至少 1 GiB `MemAvailable`，`staging-integration` 将 Spring Test context cache 限为 16；不足时 fail-fast，不得把 exit 137 当测试通过或重试掩盖。
