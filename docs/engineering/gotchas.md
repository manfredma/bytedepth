...
- staging 集成测试或 E2E 清理时，启动 app 不会自动恢复依赖 app 的 native edge；清理必须显式启动并验证 `bytedepth-staging-native-edge.service` 及其 18081 `/version`，否则共享 Nginx 仍 active 但公网请求会 502。
