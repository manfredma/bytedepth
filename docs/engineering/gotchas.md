...
- native production green 的公网 Nginx 使用 `/etc/bytedepth/production-green-public-nginx.conf`，额外站点必须放在它 include 的 `/etc/nginx/conf.d/`；旧 Docker 入口使用 `/opt/nginx-conf.d/`。只更新旧目录不会让 native Nginx 加载 staging 证书路由，未匹配的 SNI 会拿到第一个虚拟主机（例如 Daylilt）的证书，表现为生产证书不匹配。证书同步和生产发布必须同时安装 `staging-edge-certificate.conf` 与 `staging-legacy-production-entry.conf`，完成自定义配置校验和对应服务 reload。
