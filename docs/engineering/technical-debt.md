...
## TD-0006：文档-only 变更是否需要重跑 staging 验收的判定规则不明确

- 状态：`Open`
- 发现日期：2026-09-25
- 范围：staging 候选 SHA 变化后的集成测试、E2E、evidence 复用和发布流程；不改变本次版本的代码行为。
- 现状：当前规则强调 evidence 必须绑定候选完整 SHA，但没有明确区分会影响运行时制品的文档/发布元数据变更与不会影响运行时制品的纯文档变更，导致每次文档提交都可能被机械地视为必须重跑全部 staging 验收。
- 影响：发布流程可能因技术债记录、发布说明等非运行时内容重复构建和执行集成/E2E，增加发布耗时；另一方面，若放宽过度又可能错误复用与运行时制品不一致的 evidence。
- 根因：runtime manifest、构建制品、部署配置、发布 Changelog 和测试 evidence 的依赖关系尚未形成可执行的变更分类规则。
- 后续方向：发布完成后定义“影响测试结果”的变更分类，至少区分运行时代码/资源、部署脚本与配置、测试脚本/fixture、发布元数据和纯知识库文档；对不改变构建制品、部署输入和测试输入的变更，设计可审计的 evidence 复用条件，并由脚本自动校验，禁止人工口头判断。
- 验收条件：规则能够对上述变更类别给出唯一结论；运行时或测试输入变化必然作废旧 evidence；符合复用条件的纯文档变更无需重跑集成/E2E且仍能通过发布门禁；相关分类契约测试覆盖并写入发布文档。
