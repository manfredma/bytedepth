...
核对页面与 API 的响应数据不包含数据库密码、Redis 值、Meili API key、连接 URL 或异常堆栈。补充 `docs/ops.md` 的“系统运维页”段落，说明入口、最低权限、只读边界和容器/主机监控不属于该页面。

运行项目规定的 Maven 缓存刷新和完整测试。提交文档或验收中必要的最小修正。
