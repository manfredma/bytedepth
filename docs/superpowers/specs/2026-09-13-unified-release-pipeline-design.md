...
bytedepth、Career、Daylilt、Toolbox 对齐相同发布顺序、入口名称、证据格式和失败规则。GitHub Actions 只验证可在无凭据 runner 中运行的质量门禁；它绝不连接 staging、生产、Docker、数据库或共享浏览器。staging 是唯一集成与 E2E 环境，生产只接受 `main` 上受控脚本创建的新 annotated SemVer Tag。
