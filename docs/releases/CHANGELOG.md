...
- 生产发布预检兼容现网配置变量别名；在停止现役服务前规范化 root/port 配置键并校验 native 回退基线，规范化配置仍由 `ubuntu` 持有。
...
- 修复现役生产配置使用带历史命名空间的 root/port 键、导致 native 发布预检错误判定配置缺失的问题；现在会在停止任何服务前解析并规范化到 `ubuntu` 所有的 canonical 配置文件。
...
- 生产发布收敛为单一 native 路径；首轮升级统一现役 unit、配置与 release 路径，持久数据保持原位，并自动修复 current 软链接。
- 项目部署脚本、配置、发布物、日志和测试资源统一由登录用户 ubuntu 持有；自动检查拒绝 root 所有的项目资源。

### Fixed

- 修复生产发布入口依赖已退出运行环境的问题；新版本失败时恢复上一版 JAR、重启应用并 reload edge 与公网 Nginx。
...
- 修复 staging 集成测试 runner 在 `verify` 中重复运行 Surefire 单元测试、导致测试 fork 在多项目主机内存紧张时触发 global OOM 的问题；profile 现在跳过 Surefire、只运行最大堆 512 MiB 的 Failsafe `*IT`，并增加停 app 前后的内存门槛与 Spring Test context cache 限制。
- 修复 staging E2E 只检查 Playwright stdout、漏检 test-slot systemd journal WARNING 的门禁缺口；E2E evidence 现在要求两路日志都通过 warning policy。
...
**Tag**：`v2.25.14`（由本次 release:prepare 创建）
**Commit**：由本次 release:prepare 冻结
**部署**：生产 native green 公网 Nginx 整体替换；切流前停止旧 Docker Nginx，停蓝应用后重新执行最终数据导入，再启动 native 公网 Nginx。
**回滚基线**：`v2.25.2`

### Fixed

- 生产公网入口改为独立 native green Nginx systemd 服务；旧 Docker Nginx 配置保持不变，切流失败时停止新入口并启动旧入口和蓝应用回退。
- 修复切流时最终数据导入发生在公网流量停止之前的问题；现在停旧入口、停蓝应用后重新执行 final-sync，确保 green 使用停流后的最新数据。
...
- 修复发布例外契约测试的环境变量隔离，保证正常 staging evidence 门禁测试不受外层发布例外影响。
...
**Tag**：`v2.25.10`（由本次 release:prepare 创建）
**Commit**：由本次 release:prepare 冻结
**部署**：修复 v2.25.9 native green 应用因生产主机缺少 Java 25 而在切流前失败的问题。
**回滚基线**：`v2.25.2`

### Fixed

- 生产 native green 安装器现在会准备并验证 Java 25，并使用目标主机实际解析出的 Java 路径渲染 systemd 应用服务；Java 25 不可用时在 Docker blue 仍提供流量阶段 fail-fast。
...
- 修复 staging 制品上传失败或远程安装中断后残留 `/tmp/bytedepth-staging-<SHA>` 临时目录的问题，避免历史 JAR 制品占满 staging 的 `/tmp` tmpfs。
...
**Commit**：由本次 release:prepare 冻结
**部署**：staging 集成测试与 E2E 已通过，待生产验收。
**回滚基线**：`v2.25.2`
...
**Tag**：`v2.25.2`
**Commit**：`b9ffd0b5029c804f7044b52d6ebe32aa11baba0c`
**部署**：2026-09-24 07:27:50 UTC；生产单机（`175.24.197.202`）已部署并验收通过。版本接口、只读页面回归和日志门禁均通过。
**回滚基线**：`v2.25.1`
...
- staging 集成测试 12/12 通过；全量 E2E 22 个通过、12 个按既有设备条件跳过、0 个失败。

## [v2.25.1] - 2026-09-24

**Tag**：`v2.25.1`
**Commit**：`6a96c364ce78af2b9d8348240c38124744d63dbf`
**部署**：2026-09-24 05:26:06 UTC；生产部署完成，但发布后日志门禁发现既有异步执行器 WARNING 与缺失静态资源 ERROR，未记为最终验收通过。
**回滚基线**：`v2.25.0`
...
- 代码块组件兼容 Obsidian Codeblock Customizer 的 `group`/`tab` 元数据，支持 `:` 与 `=` 参数形式及省略 `tab` 时按语言名回退；同步前校验同组代码块连续性和重复有效页签。

## [v2.25.0] - 2026-09-23

**Tag**：`v2.25.0`
**Commit**：`7222c38ec962ff1dce1e55621c44baa697a46b6b`
**部署**：2026-09-23 02:56:42 UTC；生产单机（`175.24.197.202`）已部署并验收通过。完整 Compose 服务正常，生产版本接口、SNI 查询和只读查询回归均通过。
**回滚基线**：`v2.24.3`

### Changed

- 文章顶层代码块统一为可复制、带行号、可展开/收起的组件；普通代码块和未标注语言代码块保持兼容，不杜撰文件名。
- 支持显式 `tabs:` 多语言代码 Tab；每个代码面板独立维护语法高亮、行号、复制和展开/收起状态，显式 `fold` 才默认收起。
...
**Tag**：`v2.24.3`
**Commit**：`e1e6a6452c3a966d38126dd21ed05cd7c90c528f`
**部署**：2026-09-22 15:44:17 UTC；生产单机（`175.24.197.202`）已部署并验收通过。完整 Compose 服务正常，生产 SNI 查询回归通过。
**回滚基线**：`v2.24.2`

### Fixed

- 修复访问统计“全部”时间范围误显示上一周期对比曲线的问题；今天、本周、本月和本年保留同比，“全部”从配置的本站上线日（2026-06-01）开始。
